...
const config$3 = deepmergeTs.deepmerge(currying, noMutations, noExceptions, noObjectOrientation, noStatements, stylistic, overrides$1);
...
const config$2 = deepmergeTs.deepmerge(functional, overrides);
            ...
            pattern === "**"
                ...
                :
                    pattern === "*"
                        ...
                        :
                            new RegExp(`^${escapeRegExp__default["default"](pattern).replace(/\\\*/gu, ".*")}$`, "u").test(textParts[0]) &&
                                accessorPatternMatch(remainingPatternParts, textParts.slice(1), allowExtra);
}
...
function shouldIgnoreLocalMutation(node, context, { allowLocalMutation }) {
    return allowLocalMutation === true && inFunctionBody(node);
}
function shouldIgnoreClass(node, context, { ignoreClass }) {
    return ((ignoreClass === true && inClass(node)) ||
...
function shouldIgnoreInterface(node, context, { ignoreInterface }) {
    return ignoreInterface === true && inInterface(node);
}
function shouldIgnorePattern(node, context, { ignorePattern, ignoreAccessorPattern, }) {
...
function createRule(name, meta, defaultOptions, ruleFunctionsMap) {
    return utils.ESLintUtils.RuleCreator((ruleName) => `https://github.com/jonaskello/eslint-plugin-functional/blob/v4.2.2/docs/rules/${ruleName}.md`)({
        name,
        meta,
        defaultOptions,
...
const schema$f = [
    {
        type: "object",
        properties: deepmergeTs.deepmerge(ignorePatternOptionSchema, {
            allowRestParameter: {
                type: "boolean",
            },
...
const schema$e = [
    {
        type: "object",
        properties: deepmergeTs.deepmerge(ignorePatternOptionSchema, ignoreAccessorPatternOptionSchema, ignoreClassOptionSchema, {
...
function checkCallExpression$1(node, context, options) {
    ...
    const assumeTypesForArrays = assumeTypes === true ||
    ...
    const assumeTypesForObjects = assumeTypes === true ||
...
const schema$a = [
    {
        type: "object",
        properties: deepmergeTs.deepmerge(allowLocalMutationOptionSchema, ignorePatternOptionSchema, {
            allowInForLoopInit: {
                type: "boolean",
            },
        }),
        ...
        violations: carry.prevMemberType !== undefined &&
            (carry.prevMemberType !== member.type ||
                (carry.prevMemberTypeAnnotation !== member.typeAnnotation &&
                    (carry.prevMemberTypeAnnotation === utils.AST_NODE_TYPES.TSFunctionType ||
                        member.typeAnnotation === utils.AST_NODE_TYPES.TSFunctionType))),
    }), {
...
function checkTSInterfaceDeclaration(node, context, options) {
    ...
    return {
        context,
        descriptors: checkInterfaces && hasTypeElementViolations(node.body.body)
            ? [{ node, messageId: "generic" }]
            : [],
    };
}
...
const schema$1 = [
    {
        type: "object",
        properties: deepmergeTs.deepmerge(allowLocalMutationOptionSchema, ignorePatternOptionSchema, ignoreClassOptionSchema, ignoreInterfaceOptionSchema, {
...
function checkArrayOrTupleType(node, context, options) {
    ...
    if (shouldIgnoreClass(node, context, optionsObject) ||
            ...
            return [
                {
                    node,
                    messageId: "generic",
                    fix: (typeof assumeTypes !== "object" && assumingTypes) ||
